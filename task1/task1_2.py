...
lambda_1 = -10  # Observer eigenvalue 1
lambda_2 = -15  # Observer eigenvalue 2
...
Qcoeff = [1000.0, 0.0]  # State cost coefficients
Rcoeff = [0.01] * num_controls  # Control effort cost coefficients

regulator.setCostMatrices(Qcoeff, Rcoeff)
x_ref = np.array([-10, 0])  # Reference state [omega_ref, I_a_ref]
...
for k in range(num_steps):
    ...
    V_a = u_mpc[0]  # Control input (applied voltage)

    # Update the system model
    cur_y = motor_model.step(V_a)
    x_cur = motor_model.getCurrentState()  # True state of the system (unobservable in real-world scenarios)
...
plt.suptitle('Task 1.2: MPC for DC Motor Control with Observer Design', fontsize=16, fontweight='bold')
...
#------------------ Performance Metrics for Task 1.2 ------------------
# Reference is -10 rad/s
ref = -10.0
