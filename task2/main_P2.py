...
from regulator_model_P2 import RegulatorModel
...
def main():
    ...
    Qcoeff_joint_vel = [0.5] * num_controls
    ...
    B_in = {'max': np.array([10000] * num_controls), 'min': np.array([-10000] * num_controls)}
    # Define strict constraints for joints 1, 2, and 3 in turn
    # Each run should enable only one of the following blocks and comment out the others

    # --- Scenario 1: Strict constraint on Joint 1 ---
    B_out = {'max': np.array([0.6] + [10000.0] * (num_states - 1)), 'min': np.array([-0.6] + [-10000.0] * (num_states - 1)) }
    plot_label = "Strict Constraint on Joint 1"

    # --- Scenario 2: Strict constraint on Joint 2 ---
    #B_out = {'max': np.array([10000.0, 1.5] + [10000.0] * (num_states - 2)), 'min': np.array([-10000.0, -1.5] + [-10000.0] * (num_states - 2)) }
    #plot_label = "Strict Constraint on Joint 2"

    # --- Scenario 3: Strict constraint on Joint 3 ---
    #B_out = {'max': np.array([10000.0, 10000.0, 0.5] + [10000.0] * (num_states - 3)), 'min': np.array([-10000.0, -10000.0, -0.5] + [-10000.0] * (num_states - 3)) }
    #plot_label = "Strict Constraint on Joint 3"

    # Apply constraints to the regulator
    ...
    while current_time < total_time:
        ...
        print("Initial state (x0_mpc):", x0_mpc)
        print("B_out max:", B_out['max'])
        print("B_out min:", B_out['min'])
    ...
    # Plot joint positions in first figure
    ...
    plt.title(f'Joint Positions - {plot_label}')
    plt.xlabel('Time [s]')
